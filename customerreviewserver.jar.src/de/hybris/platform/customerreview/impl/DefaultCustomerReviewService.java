...
/*    */ import de.hybris.platform.customerreview.CustomerReviewUtil;
/*    */ 
/*    */ 
/*    */ 
/*    */ 
...
/*    */   {
			 ...
			 if(!CustomerReviewUtil.validateCurseWord(comment))
				throw new Exception("Error comment!");
			 //c.	Check if the rating is not < 0.  If it is < 0, throw an exception with a message.
			 if (!CustomerReviewUtil.validateRating(rating) ) {
						 throw new Exception("Error rating!");
		     }
...
/*    */   }
     ...
     /*
	  option 1: use productModel and rating range to query the table, it should be best performance
      option 2: assume all the reviews retrieved aleardy, just iterate the review list 	  
	  */
      public int getTotalReviewNumber(ProductModel paramProductModel, Double startRating,Double endRating){
		  int totalReviewNumber = 0;
		  // Option 2
		  if (startRating == null || endRating == null || startRating > endRating) { 
		      return totalReviewNumber; 
		  }
		  List<CustomerReviewModel> allReviews = getAllReviews(paramProductModel);
		  if (allReviews == null) {
			  return totalReviewNumber; 
		  }
		  for ( CustomerReviewModel review: allReviews) {
			   if (reviw != null && review.getRating() != null ) {
			         Double currentrating = review.getRating();
					 if (currentrating > startRating && currentrating < endRating ) {
						 totalReviewNumber++;
					 }
			   }	 
		  }
